...
def stream_video():
    """
    GET /api/stream?url=<link>&token=<token>
    Same as /api/download but Content-Disposition: inline so the browser
    plays the video directly instead of downloading it.
    """
    import subprocess
    from flask import Response, stream_with_context
    ...
    logger.info(f"[stream] ffmpeg: {filename}")
